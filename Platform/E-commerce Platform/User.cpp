...
void User::writeUser() {
	...
	ofp << amountOfBC << endl;
	for (i = 0; i < bankCards.size(); i++)
		bankCards[i].writeCard();
}
...
void User::setAmountOfBC(int amount) {
	amountOfBC = amount;
}

int User::getAmountOfBC(){
	return amountOfBC;
}
...
void User::clearALLShoppingCart() {
	unsigned int i, j;
	int k = 0, a;
	double sum = 0;
	int partOfC[MAXCATEGORIES];
...
void User::bind() {
	...
	while (1) {
		...
		cin >> cardID;
		for(i=0;i<bankCards.size();i++)
			if (!strcmp(cardID,bankCards[i].getCardID())) {
				cout << endl << "您已绑定该银行卡哦~(づ￣ 3￣)づ" << endl;
				return;
			}
		cout << endl;
		for (i = 0; i < Card::getBanks().size(); i++)
			cout << i + 1 << "." << Card::getBanks()[i] << endl;
		cout << endl << "请选择所属银行序号：" << endl;
		cin >> choice;
		buffer = socket(0, cardID, password);
		if (buffer==1) {
			cout << endl << "请输入银行卡密码：" << endl;
			while (1) {
				cin >> password;
				buffer = socket(0, cardID, password);
				if (buffer==1) {
					...
					card.setBank(Card::getBanks()[choice - 1]);
					...
					cout << endl << "密码有误，请您重新输入：" << endl;
				else {
					cout << endl << "网络连接存在问题，请您检查后重新绑定(づ￣ 3￣)づ" << endl;
					return;
				}
			}
			break;
			...
			cout << endl << "该银行卡不存在，请您填写正确信息！" << endl;
		else {
			cout << endl << "网络连接存在问题，请您检查后重新绑定(づ￣ 3￣)づ" << endl;
			return;
		}
...
void User::delPartOfBC() {
	unsigned int i, j;
	string choice;
	vector<string> choices;
	vector<Card>::iterator iter;
	cout << endl << "请依次输入需解绑的银行卡序号，以空格为分界符：" << endl;
	getline(cin, choice);
	getline(cin, choice);
	splitString(choice, choices, " ");
	for (i = 0; i < choices.size(); i++) {
		iter = bankCards.begin();
		for (j = 0; j < atoi(choices[i].c_str()) - 1 - i; j++)
			iter++;
		bankCards.erase(iter);
	}
	cout << "..." << endl;
	Sleep(3000);
	cout << "解绑成功！" << endl;
	amountOfBC -= choices.size();
}

int User::pay(double sum) {
	int choice;
	int buffer;
	int times = 0;
	string password = "000";
	...
	while (1) {
		cin >> password;
		if (password != bankCards[choice - 1].getPassword())
			cout << "密码有误！请您重新输入：" << endl;
		else
			break;
	}
	while (1) {
		if (choice <= amountOfBC) {
			buffer = socket(sum, bankCards[choice - 1].getCardID(),"000");
			if (buffer == 1) {
				return 1;
			}
			else if(buffer == 0){
				times++;
				if (times >= amountOfBC) {
					cout << endl << "您的支付宝已被掏空，无法完成交易，请充值后再来哦~(づ￣ 3￣)づ" << endl;
					return 0;
				}
				else
					cout << endl << "您的余额不足！请另选银行卡：" << endl;
			}
			else {
				cout << endl << "网络连接存在问题，请您检查后重新支付(づ￣ 3￣)づ" << endl;
				return 0;
			}
		}
		else
			cout << endl << "银行卡序号输入有误！请您重新输入：" << endl;
	}
}
...
void User::priCard() {
	...
	cout << "******************************************************************************************" << endl;
	for (i = 0; i < bankCards.size(); i++) {
		cout << i + 1 << "." << "所属银行：" << bankCards[i].getBank() << endl;
		cout << "卡号：";
		for (j = 0; j < 4; j++) {
			for (k = 0; k < 4; k++)
				cout << bankCards[i].getCardID()[k + j * 4];
			cout << " ";
		}
		cout << bankCards[i].getCardID()[16] << bankCards[i].getCardID()[17] << bankCards[i].getCardID()[18] << endl << endl;
	}
}
