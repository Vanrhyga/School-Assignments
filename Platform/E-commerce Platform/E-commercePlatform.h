...
private:
	...
	char cardID[20];						//卡号
	string bank;							//所属银行
	string password;						//支付密码
public:
	static void readBank();					//从文件读取银行名称
	static vector<string> getBanks();		//获得银行名称
	void setCardID(char*);					//设置银行卡号
	void setBank(string);					//设置所属银行
	void setPassword(string);				//设置支付密码
	string getBank();						//获得所属银行
	char* getCardID();						//获得卡号
	string getPassword();					//获得密码
	void readCard();						//从文件读取
	void writeCard();						//向文件写入银行卡信息
	Card();
	~Card();
};
...
private:
	...
	int amountOfBC;							//银行卡总数
	vector<Card> bankCards;					//银行卡
public:
	static void readAnnotation();			//跳过注释信息
	static void writeAnnotation();			//向文件写入注释信息
	void read();							//读取信息
	...
	void setAmountOfBC(int);				//设置银行卡总数
	int getAmountOfBC();					//获得银行卡总数
	...
	void clearPartOfSC();					//部分结算
	void clearALLShoppingCart();			//全选结算
	...
	void priUser();							//打印用户信息
	void bind();							//绑定银行卡
	void delPartOfBC();						//解绑部分银行卡
	void priCard();							//打印银行卡信息
	int pay(double);						//支付
	User();
	~User();
};
